...
_Sticky pagination, single-page selector, and LeadsTab horizontal scroll (`fix/sticky-pagination-and-leads-scroll`, 4 commits, no migrations, **on branch, not merged**, 9 Aug 2026):_
...
- **A single-page result no longer hides the page size selector.** Previously the whole bar collapsed to a count line, so there was no route from "39 rows at 50 per page" back to a paged view except widening the result set. Now a single page keeps the selector and drops only the nav and the count. Verified live by dropping 40 rows from 50 to 10 per page and watching the list become paged. Zero items still hides everything, since the caller's empty state should own the screen. **Trade-off accepted knowingly:** single-page results lose the "Showing 1 to 4 of 4 leads" line, which was the explicit ask; it is one branch in `Pagination.tsx` if it should come back alongside the selector.
...
- **Still outstanding before merge:** the incognito pass required by Section 13, and the same `EnquiriesTab` multi-page gap as the parent branch (one enquiry in the database).

_Shared pagination across Activity, Leads and Enquiries (`feature/shared-pagination`, 5 commits, no migrations, **on branch, not merged**, 8 Aug 2026):_
...
- **Still outstanding before merge:** the incognito pass required by Section 13, and `EnquiriesTab`'s multi-page behaviour, which could not be exercised because the database holds exactly one enquiry (the component itself is the same one verified extensively on the other two tabs).
