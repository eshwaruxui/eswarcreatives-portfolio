...
| send-confirmed-outreach-touches | v1 | **false** | Cron-invoked (pg_cron/pg_net every 5 min), auth via `x-cron-secret` header (not a user JWT — verify_jwt must stay false). Finds confirmed touches whose held delivery window has arrived, re-runs all safety checks, sends via Resend with the same fixed `htmlBody()`. Does **not** itself re-validate business hours — trusts `scheduled_for` completely, which is why the `scheduled_for` precision/corruption issues in Section 1's PR #19 entry were urgent. New 4 Aug 2026, see Section 1 |
...
| Resend webhook for Opened/Bounced tracking — `resend-outreach-webhook` is fully built (signature verification, idempotent-by-first-open, bounce→suppression+lead-status+cancel-remaining-touches) but confirmed via `get_logs` on 7 Aug 2026 to have **zero invocations in the last 24h**: the webhook was never registered in the Resend dashboard, and `RESEND_WEBHOOK_SECRET` was never set in Supabase (would 401 every call regardless). Also blocks Activity tab's Opened/Bounced columns from being useful sort candidates (left unsortable 7 Aug 2026 for exactly this reason, see Section 1's PR #21 entry). Broader hardening plan discussed 7 Aug 2026, not yet started: check DB write results in the handler (currently ignored — a failed write still returns 200, so Resend never retries and the event is lost silently), add `svix-timestamp` replay-window validation, dedupe by `svix-id` via a small `webhook_events` table, add `email.complained` (treat like a hard bounce) and `email.clicked` (stronger buying-intent signal than an open) handling, and consider a normalized `outreach_touch_events` table (full event history) with `opened_at`/`bounced_at`/`clicked_at` kept as denormalized first-occurrence columns for fast list sorting — lays groundwork for the "open rate analytics" Phase 2 item below instead of a second one-off patch later | High |
| Invoice nudge automation (next available migration is `0093`): scheduled reminders at due date, +3d, +7d with PDF attachment | High |
