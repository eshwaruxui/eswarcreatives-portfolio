...
_Shared pagination across Activity, Leads and Enquiries (`feature/shared-pagination`, 4 commits, no migrations, **on branch, not merged**, 8 Aug 2026):_
...
| `LeadsTab`'s 10-column desktop table overflows its 961px wrapper horizontally at a 1280px viewport (206px over at 25 rows per page). Pre-existing and content-driven, not caused by pagination — measured 8 Aug 2026: overflow *grows* with row count (178px at 10 rows, 261px at 100), so the previously unpaginated 267-row table was worse and pagination strictly reduced it. The table's own `minWidth` is 860, well under the wrapper, so the fix is column-level (pin or drop a column, or apply `FadeOverflow` to the widest cells) the same way `ActivityTab` was fixed on 8 Aug | Medium |
