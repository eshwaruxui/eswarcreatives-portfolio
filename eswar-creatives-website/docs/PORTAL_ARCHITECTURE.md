...
Last updated: 6 August 2026 (PR #20 — stale Due Today fix, a unified intent/step label source of truth surfaced across the Outreach preview title and every Today Due/Review in Advance/Scheduled row, the Review in Advance placeholder-time fix, and a WCAG AA contrast fix to `t.text.muted`, merged staging → main). Keep this in the repo at `docs/PORTAL_ARCHITECTURE.md` and point Claude Code at it before starting any portal work.
...
| confirm-scheduled-touch | v10 | true | Admin approves a "Review in Advance" touch — does **not** send. Validates lead (email, suppression, observation), stamps draft_confirmed_at/by, pushes scheduled_for via the shared `_shared/businessHours.ts` recipient-local 9:30 AM-5:30 PM Mon-Fri policy (same module `send-outreach-email` uses — previously each function had its own separate, inconsistent logic). v10 (6 Aug 2026) switched to the shared module — see Section 1's PR #19 entry |
...
| Item | Priority |
|---|---|
| "Review in Advance" hold + cron auto-send, now on the unified business-hours engine (6 Aug 2026) — auth handshake and empty-queue path verified via curl, and the scheduling math/data itself has been directly verified in the database, but no real approve-then-auto-send has been watched leave the outbox at its actual 9:30 AM local window yet; test with a real lead before trusting with live prospects | High |
