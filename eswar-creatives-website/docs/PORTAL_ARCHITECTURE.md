...
- **Column sizing:** Lead pinned to 140px, action column to 180px (both via `SortableColumn.width`, which already existed, plus matching `minWidth` on the `<td>`s), with **Status left as the flexible column** — it's the only cell whose content length varies wildly, so it absorbs the slack. Table `min-width` raised 760 → 960 to match the real sum of those columns, so a narrow container scrolls instead of crushing the buttons; at 1280px the admin content area is ~1040px, so nothing scrolls. The action cell already had `justify-content: flex-end`, so approved rows showing Preview alone still sit flush right.
